...
class RecibosPlanesFamiliares {
  ...
  generateReceipt() {
    ...
    let btnGenerateReceiptFunction = (event) => {
      ...
      totalDsctoBeneficioXLineaAdicional = Number(valNumbaselines.value) === 0 ? (Number(dsctoPrimerRecibo) * (Number(valNumnewlines.value) - 1)) : ((Number(dsctoPrimerRecibo) * Number(valNumbaselines.value)) * Number(valNumnewlines.value));

      let cuentaCliente = `<div class="descripcion-plan-cuenta-cliente bg-DCECF4">
            <p class="e-tc--blue">Descuento General</p>
            <p class="e-tc--blue"></p>
            <p class="e-tc--blue">${parseFloat(totalDsctoBeneficioXLineaAdicional + dsctoPlanFamiliar).toFixed(2)}</p>
          </div>
      ...
      for (let w = 0, l = parseInt(valNumbaselines.value); w < l; w++) {
        ...
        let calcularDevolucionProrrateo = parseFloat(
          parseFloat(calcularProrrateo) * -1
        ).toFixed(2);
